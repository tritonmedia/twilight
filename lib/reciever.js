...
const getName = (name, originalName) => {
  const matches = /[e ](\d+)/gi.exec(originalName)
  const num = parseInt(matches[1], 0)

  if(originalName.indexOf('NCED') !== -1 || originalName.indexOf('NCOP') !== -1) {
    debug('getName', 'skipping NCED/OP', originalName)
    return null
  }
...
module.exports = async (config, queue) => {
  ...
  const exists = async (name, type) => {
    ...
    return await fs.pathExists(loc)
  }
